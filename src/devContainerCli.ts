...
function buildWslCliInvocation(
  ...
  // Use CLI version based on @devcontainers/cli dependency in package.json, stripped of semver prefix.
  ...
  const version = range.replace(/^[^\d]*/u, "");
  ...
  const script = [
    ...
    "fi",
    // Invoke dev container CLI by its absolute path rather than relying on PATH resolution: some
...
function runCliCapture(
  ...
  const wslInfo = cwd ? parseWslInfo(cwd) : undefined;
  if (wslInfo) {
    ...
    const wslArgs = args.map((arg) => parseWslInfo(arg)?.linuxPath ?? arg);
    const {
      command,
      args: cmdArgs,
      input,
    } = buildWslCliInvocation(ctx, wslInfo.distro, wslArgs);
...
// object carrying the given key, ideally.
...
function parseDevcontainerUpResult(output: string) {
...
export async function readMergedCustomizations(
  ...
  // metadata, and every feature's customizations. `customizations.vscode` is an
  // array of { extensions, settings } entries, so we concatenate the extension lists
  // and merge the settings objects in source order.
